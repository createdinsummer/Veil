...
工作区容器使用 Argon2id 从用户密码和 salt 派生密码保护密钥，元数据中保存随机数据主密钥。文件内容使用数据主密钥进行分块流式认证加密，元数据使用密码保护密钥加密。默认修改密码只重新保护元数据；`veil passwd --full` 会轮换数据主密钥并流式重写全部文件。
...
- 同一工作区没有跨进程写锁。
- `passwd` 默认只修改密码保护层；`passwd --full` 才会重新加密工作区中的全部文件。
