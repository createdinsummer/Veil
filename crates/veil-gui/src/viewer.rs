...
//!
//! [`ViewerRegistry`] 按 `FsNode.mime` 找第一个能处理的查看器；找不到 → 由 UI 走
//! 「导出到本地」兜底。渲染层只调 `veil-core` 的数据能力，不碰加解密逻辑。

use anyhow::Result;
use eframe::egui;
use veil_core::container::Container;
use veil_core::index::FsNode;
...
pub trait Viewer {
    /// 能否处理这个文件（按 MIME 判断）。
    fn can_handle(&self, node: &FsNode) -> bool;
    /// 打开它，返回活动视图。
    fn open(
        &self,
        container: &Container,
...
impl ViewerRegistry {
    ...
    pub fn find(&self, node: &FsNode) -> Option<&dyn Viewer> {
        self.viewers.iter().map(|v| v.as_ref()).find(|v| v.can_handle(node))
    }
}
...
struct ImageView {
    texture: egui::TextureHandle, // Drop 时自动释放显存
}

impl Viewer for ImageViewer {
    fn can_handle(&self, node: &FsNode) -> bool {
        node.mime.as_deref().is_some_and(|m| m.starts_with("image/"))
    }
    ...
    fn open(
        ...
        let bytes = container.read_file(&node.path)?; // 解密到内存（不落盘）
        let img = image::load_from_memory(&bytes)?; // 解码
        let rgba = img.to_rgba8();
        let size = [rgba.width() as usize, rgba.height() as usize];
        let color = egui::ColorImage::from_rgba_unmultiplied(size, rgba.as_raw());
        let texture = ctx.load_texture(node.path.clone(), color, egui::TextureOptions::LINEAR);
...
struct MediaView {
    // 持有 RAII 守卫：视图存活期间临时明文在，Drop（切换文件/锁定）即删除
    _temp: TempPlaintext,
}
...
impl Viewer for MediaViewer {
    ...
    fn open(
        ...
        let temp = container.extract_to_temp(&node.path)?; // 解密到受控临时位置（优先 RAM）
