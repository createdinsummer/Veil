...
impl DisplayEncoding {
    ...
    pub fn name(&self) -> &str {
        match self {
            DisplayEncoding::Utf8 => "UTF-8",
            DisplayEncoding::Gbk => "GBK",
            DisplayEncoding::Gb2312 => "GB2312",
            DisplayEncoding::ShiftJis => "Shift-JIS",
            DisplayEncoding::Big5 => "Big5",
            DisplayEncoding::Unknown(_) => "Unknown",
        }
    }
...
pub fn get_display_encoding() -> DisplayEncoding {
...
/// 将多个 UTF-8 字符串批量编码（跨平台）
///
/// 适用于需要批量转换的场景，如 UI 界面渲染
///
/// # 示例
/// ```rust
/// let messages = vec!["标题", "内容", "按钮"];
/// let encoded = encode_batch_for_display(&messages);
/// for bytes in encoded {
///     ui.render(&bytes);
/// }
/// ```
pub fn encode_batch_for_display(utf8_strings: &[&str]) -> Vec<Vec<u8>> {
    utf8_strings
        .iter()
        .map(|s| encode_for_display(s))
        .collect()
}

/// 将显示编码的字节转换回 UTF-8（跨平台）
///
/// 用于从用户输入或外部数据读取后转换回程序内部的 UTF-8
///
/// # 注意
/// - 这个函数用于用户交互输入，不应该用于文件内容
/// - 文件内容应该保持原始字节，不进行解码
///
/// # 示例
/// ```rust
/// let input_bytes = read_from_user();
/// let utf8_string = decode_from_display(&input_bytes)?;
/// ```
pub fn decode_from_display(bytes: &[u8]) -> io::Result<String> {
    let encoding = get_display_encoding();
    ...
    // 否则进行解码
    convert_encoding_to_utf8(bytes, encoding)
}

/// 获取当前编码的代码页 ID（用于调试/日志）
pub fn get_code_page_id() -> u32 {
    match get_display_encoding() {
        DisplayEncoding::Utf8 => 65001,
...
// Windows: 解码
#[cfg(target_os = "windows")]
fn convert_encoding_to_utf8(bytes: &[u8], encoding: DisplayEncoding) -> io::Result<String> {
    use winapi::um::stringapiset::{MultiByteToWideChar, WideCharToMultiByte};
    use winapi::um::winnls::CP_UTF8;

    unsafe {
        // 步骤 1: 源编码 → UTF-16
        let source_cp = match encoding {
            DisplayEncoding::Utf8 => CP_UTF8,
            DisplayEncoding::Gbk => 936,
            DisplayEncoding::Gb2312 => 20936,
            DisplayEncoding::ShiftJis => 932,
        ...
        // 步骤 2: UTF-16 → UTF-8
        let utf8_len = WideCharToMultiByte(
            CP_UTF8,
            0,
            wide_buf.as_ptr(),
            wide_buf.len() as i32,
        ...
        WideCharToMultiByte(
            ...
            wide_buf.as_ptr(),
            wide_buf.len() as i32,
            utf8_buf.as_mut_ptr() as *mut i8,
            utf8_len,
            std::ptr::null(),
            std::ptr::null_mut(),
        );

        String::from_utf8(utf8_buf)
            .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))
    }
}
...
// Unix/Linux/macOS: 解码
#[cfg(not(target_os = "windows"))]
fn convert_encoding_to_utf8(bytes: &[u8], _encoding: DisplayEncoding) -> io::Result<String> {
...
/// CLI: 输出到 stdout
pub fn cli_print(s: &str) -> io::Result<()> {
    use std::io::Write;
    let bytes = encode_for_display(s);
    io::stdout().write_all(&bytes)?;
    io::stdout().flush()
}

/// CLI: 输出到 stdout 并换行
pub fn cli_println(s: &str) -> io::Result<()> {
    cli_print(s)?;
    cli_print("\n")
}

/// CLI: 输出到 stderr
pub fn cli_eprint(s: &str) -> io::Result<()> {
    use std::io::Write;
    let bytes = encode_for_display(s);
...
/// CLI: 输出到 stderr 并换行
pub fn cli_eprintln(s: &str) -> io::Result<()> {
    cli_eprint(s)?;
    cli_eprint("\n")
}
...
mod tests {
    ...
    fn test_batch_conversion() {
        let strings = vec!["测试1", "测试2", "测试3"];
        let encoded = encode_batch_for_display(&strings);
