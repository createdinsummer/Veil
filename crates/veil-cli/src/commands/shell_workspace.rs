...
pub fn run_workspace(container_name: &str, password: Option<String>) -> Result<()> {
    ...
    loop {
        ...
        match cmd {
            ...
            "add" => {
                if parts.len() < 2 {
                    crate::outln!("{}", crate::i18n::t("shell.usage_add").yellow());
                    continue;
                }
            ...
            "rm" | "delete" => {
                if parts.len() < 2 {
                    crate::outln!("{}", crate::i18n::t("shell.usage_rm").yellow());
                    continue;
                }
            ...
            "ex" | "extract" => {
                if parts.len() < 3 {
                    crate::outln!("{}", crate::i18n::t("shell.usage_ex").yellow());
                    continue;
                }
                let file_name = parts[1];
            ...
            _ => {
                crate::outln!("{}", crate::i18n::t1("shell.unknown", "cmd", cmd).yellow());
            }
        }
    }
...
fn print_help() {
    ...
    crate::outln!("{}", crate::i18n::t("shell.help_title").bright_cyan());
    crate::outln!(
        "  {}  - {}",
        "ls".bright_white(),
...
fn list_files(manager: &WorkspaceManager, password: &str) -> Result<()> {
    ...
    // 空容器只显示空状态，不打印表头。
    if files.is_empty() {
        crate::outln!("{}", crate::i18n::t("common.empty").bright_black());
        return Ok(());
    }
